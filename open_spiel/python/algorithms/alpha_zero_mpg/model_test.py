...
def build_model(game, model_type):
  config=alpha_zero.Config(game=game, model=model_type, nn_width=32, nn_depth=2, weight_decay=1e-4, learning_rate=0.01, path=None)
  return model_lib.MPGModel(config=config,game=game)


class ModelTest(parameterized.TestCase):

  @parameterized.parameters(model_lib.valid_model_types)
  def test_model_learns_simple(self, model_type):
    game = pyspiel.load_game("tic_tac_toe")
    model = build_model(game, model_type)
    print("Num variables:", model.count_trainable_variables())
