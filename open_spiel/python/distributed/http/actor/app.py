...
import open_spiel.python.algorithms.alpha_zero_mpg.multiprocess.orchestrator as orchestrator_lib
...
class HttpActorFactory:
    ...
    def __call__(self, config, game, queue, hostname, num):
        act = actor.MultiProcActor(config, num, name=f"actor-{hostname}")
...
class ActorApp(common.AlphaZeroService):
    ...
    def start(self):
        ...
        if isinstance(config.services.actors.instances, int):
            self.actors = [spawn.Process(self.actor_factory, kwargs={"game": game, "config": config,
                                                           "num": i,"hostname":socket.gethostname()})
    ...
    def __init__(self):
        ...
        self.actor_factory=HttpActorFactory(None)
        self.actors=[]
        config=self.config
        self._started = False
...
def health() -> health_dto.NodeHeartbeatResponse:
    ...
    time.sleep(app.config.heartbeat_response)
